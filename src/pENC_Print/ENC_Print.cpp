...
ENC_Print::ENC_Print()
{
  ...
  N_lat = 43.076; //43.07511878;
  E_long = -70.683; //-70.68395689;
  S_lat = 43.0589; //43.05780589;
  W_long = -70.725; //-70.72434189;
...
bool ENC_Print::OnNewMail(MOOSMSG_LIST &NewMail)
{
    ...
    for(p=NewMail.begin(); p!=NewMail.end(); p++)
    {
        ...
        else if (name == "ASV_DRAFT")
            m_ASV_draft = atof(msg.GetString().c_str());
    }
...
void ENC_Print::printPoints()
{
    ...
    while(feature)
    {
        ...
        if ((first_print) || (MLLW_t_lvl != t_lvl))
        {
            ...
            //cout << t_lvl <<"\t" << location<< endl;
            //cout << to_string(t_lvl)+", "+to_string(depth) << endl;
...
{
    OGRFeature *feature;
    OGRGeometry *geom, *geomIntersection, *singleGeomIntersect;
    OGRMultiPolygon *multipoly;

    int MLLW_t_lvl, WL, t_lvl, depth;

    int i =0;
    string obs_type, geomName;
    ...
    while(feature)
    {
        ...
        // If we are not printing all of the objects in the ENC, then only print the area intersected
        //  by the print area and the ENC-object. Otherwise use the full polygon geometry.
        if (!print_all)
            geomIntersection = geom->Intersection(m_filter);
        else
            geomIntersection = geom;
        ...
        if ((first_print) || (MLLW_t_lvl != t_lvl))
        {
            geomName = geomIntersection->getGeometryName();
            //cout << geomName << endl;
            if (geomName=="MULTIPOLYGON")
            {
                //cout << "Multi " << multipoly->getNumGeometries() << endl;
                multipoly = (OGRMultiPolygon *) geomIntersection;
                for(int i=0; i<multipoly->getNumGeometries(); i++)
                {
                    singleGeomIntersect= multipoly->getGeometryRef(i);
                    p(singleGeomIntersect, t_lvl, obs_type,i);
                }
            }
            else
                p(geomIntersection, t_lvl, obs_type,i);
...
void ENC_Print::p(OGRGeometry *geomIntersection, int t_lvl, string obs_type, int i)
{
    OGRPolygon *poly = (OGRPolygon *)geomIntersection;
    OGRLinearRing *ring;

    int numPoints;

    string color, label, print_poly, sizes, vertex, active;
