  ...
  IvPBehavior(gdomain)
{
  ...
  addInfoVars("Next_WPT, Obstacles, NAV_SPEED, NAV_X, NAV_Y, NAV_HEADING");
  ...
  if((param == "vehicle_size") || (param == "vehicle_length" ) || (param == "asv_length" ) && (isNumber(val)))
    {
      ...
      postMessage("ASV_Length", m_v_length);
      return(true);
    }
...
void BHV_OA_pnt::calcGaussWindow(vector<double> & OA_util, Point & Obstacle)
{
  ...
  if (Obstacle.getCost() <50)//> m_maxutil)
    {
      amplitude = 0;//m_maxutil;
      sigma = (m_maxutil-Obstacle.getCost())/(6.25);
    }
  else
    {
  ...
  for (int k = 0; k< (2*width+1); k++)
    {
      ...
      utility = calc_Gaussian(cur_ang, Obstacle.getAngle(), sigma, amplitude);
...
{
    // To make a Gaussain Depression, the gaussian is subracted by m_maxutil. The amplitude
    //  also needs to be subtacted by m_maxutil to allow for the bottom value of the Gaussian
    //  to be the correct value.
    return m_maxutil-pow(M_E, -(pow((x - mu),2)/(2*(sigma * sigma))))*(m_maxutil-amplitude);
}
