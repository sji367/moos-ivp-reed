...
void BHV_OA_poly::getVertices(int i, string info, Poly& min_angle, Poly& min_dist, Poly& max_angle, vector<double>& max_cost, bool &inside)
{
  ...
  min_angle.setMB_Right_pnt(min_dist.getCost(), min_dist.getAngle());
  max_angle.setMB_Left_pnt(min_dist.getCost(), min_dist.getAngle());
...
double BHV_OA_poly::calcBuffer(double utility)
{
  double temp_buff = 0;
  double buffer_width = 20;
  double util2increaseBuff = 30;
  double maxBuff = 50;

  // If the utility is less than 100, increase the size of the
  //  safety buffer
  if (utility < util2increaseBuff)
    {
      temp_buff = floor(util2increaseBuff-utility);
      if (temp_buff+buffer_width >maxBuff)
        temp_buff =50-buffer_width;
...
void BHV_OA_poly::calcVShape(double buffer_width, vector<double> &OA_util, Poly min_angle, Poly min_dist, Poly max_angle, bool &inside)
{
    ...
    //postMessage("THREAT", to_string(min_angle.getTLvL())+", "+to_string(min_dist.getTLvL())+", "+to_string(max_angle.getTLvL()));
    ...
    if (inside)
    {
        ...
        {
            start_ang = min_angle.getAngle();//min_angle.getAngle();
            end_ang = max_angle.getAngle();//max_angle.getAngle()+360;
            Case = ", 1";
        }
        // Case 2: (min_ang < max_ang)
        else
        {
            start_ang = max_angle.getAngle();
            end_ang = min_angle.getAngle()+360;
            Case = ", 2";
        }
    ...
    else
    {
        ...
        if (!m_SafetyDistSet)
            safety_distance = m_v_length/2*(1+t_lvl);
        else
            safety_distance = m_Safety_Dist+.5*t_lvl;
        ...
        //  leaves atleast a 10 degree gap.
        if ((min_angle.getCost() == 0)&&(max_angle.getCost() == 0))
            crossOver( min_angle.getAngle(),  max_angle.getAngle(), buffer_width);

        string angles = "BW: " + to_string(buffer_width)+"\t ";
        double minA = fmod(min_angle.getAngle()-buffer_width,360);
        double maxA = fmod(max_angle.getAngle()-buffer_width,360);
        if (minA <0)
            angles+= to_string(minA+360)+", ";
        else
            angles+= to_string(minA)+", ";

        if (minA <0)
            angles+= to_string(min_dist.getAngle()+360)+", ";
        else
            angles+= to_string(min_dist.getAngle())+", ";

        if (maxA <0)
            angles+= to_string(maxA+360);
        else
            angles+= to_string(maxA);

        postMessage("Angles1", angles);
        ...
        for (double x1 = min_angle.getAngle()-buffer_width; x1<=min_dist.getAngle(); x1++)
        {
            ...
            utility = actual_cost;//m_maxutil-actual_cost;
...
double BHV_OA_poly::crossOver(double minA, double maxA, double &buffer_width)
{
    ...
    postMessage("Buffer_Width", "BW: " + to_string(buffer_width)+ " AG: " + to_string(angleGap_raw)+" AG_Buff: " + to_string(angleGap_buffered));
}
