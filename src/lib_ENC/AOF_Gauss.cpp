...
double AOF_Gauss::evalPoint(vector<double> point) 
{
  ...
  if (temp_cent.size() == temp_sigma.size() and temp_sigma.size() == temp_range.size())
    {
  ...
  // If the sizes of cent and sigma are not the same post an error and break
  else
    {
      postWMessage("ERROR --> Size of cent != Size of sigma != Size of range");
      return 0;
    }
  //*/
}
