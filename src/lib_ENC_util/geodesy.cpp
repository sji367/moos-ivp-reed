...
void Geodesy::Initialise(double Lat_Origin, double Lon_Origin)
{
  ...
  UTM_Zone=getUTMZone(LonOrigin);
  
  UTM.SetWellKnownGeogCS("WGS84");
  UTM.SetUTM(UTM_Zone, signbit(-LatOrigin));
...
{
    OGRCoordinateTransformation* coordTrans = OGRCreateCoordinateTransformation(&LatLong, &UTM);
    if (LatLong_geom->transform(coordTrans)==OGRERR_NONE)
        return LatLong_geom;
    else
    {
...
/*
    ...
    utm.SetWellKnownGeogCS("WGS84");
    utm.SetUTM(UTM_Zone, signbit(-LatOrigin));
    OGRSpatialReference *temp_utm = &utm;
    if (LatLong_geom->transformTo(temp_utm) == OGRERR_NONE)
        return LatLong_geom;
    else
    {
