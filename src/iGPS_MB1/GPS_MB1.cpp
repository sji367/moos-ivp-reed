...
bool CGPS_MB1::OnStartUp()
{
  ...
  //port = 56004
  /* Removed to allow for serial reading
...
/*
void CGPS_MB1::ProcessGPS(string &Packet)
{
  ...
  if (HypackMode) {
    ...
    // There can potentially be multiple messages per packet
    string pThisMessage;
    pThisMessage = strtok(Packet, "\r\n");
...
bool CGPS_MB1::ParseNMEAString(string &sNMEAString)
{
  ...
  if (!DoNMEACheckSum(sCopy)) {
    MOOSDebugWrite("GPS Failed NMEA check sum "+sCopy);
    return false;
  }
  ...
  if (sWhat == "$GPGGA") {
    ...
    if (m_Geodesy.LatLong2LocalUTM(dfLatDecDeg, dfLongDecDeg, dfNLocal, dfELocal)) {
      ...
      MOOSTrace("N: "+doubleToString(dfNLocal)+ ", E: " + doubleToString(dfELocal));
      if (bGood) {
	MOOSTrace("inside NE");
