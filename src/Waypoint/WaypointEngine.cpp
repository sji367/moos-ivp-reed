...
//         "advanced"
//
//  Similar to setNextWaypoint(), setWaypoint() is called when a specific
//  waypoint index has been specified, allowing a non-sequential
//  advancement of waypoints. Waypoints may be specified by a MOOS post:
//  WPT_UPDATE = "currix=NUM", where NUM is the zero referenced waypoint
//  index. When received, the new index is specified and a flag is set 
//  that triggers setWaypoint() (this function) to be called in lieu of 
//  setNextWaypoint(). 
// 
//  This function handles house-keeping tasks such as  rotation of indices 
//  when perpetual=true, ensures the previous point is correctly specified 
//  and resets the CPA calculation. 

string WaypointEngine::setWaypoint(double os_x, double os_y)
{
  ...
  // Phase 2: Handle waypoint advancement
  // --------------------------------------------------------------
  // If waypoint has advanced, either
  // (1) just increment the waypoint, or
  // (2) start repeating the waypoints if perpetual or repeats !=0, or
  ...
  // Make sure that the previous index is correct
  if(m_curr_ix == 0)
    m_prev_ix = vsize - 1;
  else
    m_prev_ix = m_curr_ix - 1;
  ...
  double pt_x = m_seglist.get_vx(m_prev_ix);
  double pt_y = m_seglist.get_vy(m_prev_ix);
  XYPoint prev_pt(pt_x,pt_y);
  setPrevPoint(prev_pt);

  // Set the current_cpa to -1 so the very next distance
  // calculated will be treated as the minimum. 
  m_current_cpa = -1;
  return("advanced");
}
