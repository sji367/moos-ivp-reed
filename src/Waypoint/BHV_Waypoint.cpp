  ...
  IvPBehavior(gdomain)
{
  ...
  // Flag for changing waypoint index
  m_flag_update_WTP = false;
}
...
bool BHV_Waypoint::setParam(string param, string param_val) 
{
  ...
  //--------------------------------------------------------------------//
  //--------------------------------------------------------------------//
  // Modified by S. Reed and V. Schmidt, CCOM/UNH, 7/1/16
  //
  // This variable allows you to update the waypoint index by posting 
  // UPDATE_WPT = "currix=#" to the MOOSDB, where the # is replaced with 
  // the waypoint index that you want to go to next. Once this has been
  // posted to the MOOSDB, a flag (m_flag_update_WTP) will updated.
  //
  // m_flag_update_WTP is used in the function setNextWaypoint and
  // tells the function to use the setWaypoint function instead of the
  // setNextWaypoint function for m_waypoint_engine.
  else if((param == "currix") && (dval > 0)) {
    m_waypoint_engine.setCurrIndex((unsigned int)(dval));
    m_flag_update_WTP = true;
    return(true);
  }
  //--------------------------------------------------------------------//
  //--------------------------------------------------------------------//
...
IvPFunction *BHV_Waypoint::onRunState() 
{
  ...
  // Modified by S. Reed and V. Schmidt, CCOM/UNH, 7/1/16
  //
  // This block of code sets the location of the previous waypoint
  // when a waypoint index has been arbitrarily selected through a
  // MOOSDB post of UPDATE_WPT = currix=#.
  // There are three cases:
  // 1) If the selected waypoint is not the first in the list, you
  // simply select the previous waypoint.
  // 2) If the selected waypoint is the first in the list (index = 0)
  // and vehicle has finished at least one iteration of waypoints, the
  // previous waypoint is set to the last waypoint in the list.
  // 3) If the selected waypoint is the first in the list (index = 0)
  // and the vehicle is proceding on its first iteration of waypoints,
  // the previous waypoint is set to the vehicle's current position.

  
  double this_x = 0;
  double this_y = 0;

  if (m_flag_update_WTP==true)
    {
      int idx = m_waypoint_engine.getCurrIndex();
      if (idx > 0)
	{
	  this_x = m_waypoint_engine.getPointX(idx-1);
	  this_y = m_waypoint_engine.getPointY(idx-1);
	}
      else if (m_waypoint_engine.getCycleCount() > 0)
	{
	  // If the index for waypoint is 0, then the previous index
	  // is the maximum
	  if (idx == 0) 
	    {
	      idx = m_waypoint_engine.size();
	      this_x = m_waypoint_engine.getPointX(idx-1);
	      this_y = m_waypoint_engine.getPointY(idx-1);
	    }
	}
    ...
    {  
      this_x = m_waypoint_engine.getPointX();
      this_y = m_waypoint_engine.getPointY();
    }
  ...
  // Possibly increment the waypoint index
...
bool BHV_Waypoint::setNextWaypoint()
{
  ...
  // Modified by S. Reed and V. Schmidt, CCOM/UNH, 7/1/16
  //
  // In this code block, we execute the proper function to select the
  // next waypoint. If the waypoint was arbitrarily selected
  // (m_flag_update_WTP = true), we execute setWaypoint instead of
  // setNextWaypoint. setWaypoint does not determine if the ASV has
  // arrived at the waypoint before selecting the new
  // waypoint. However, both functions return a feedback message that
  // will either be: empty_seglist, completed, cycled, advanced, or
  // in-transit.
  ...
  if (m_flag_update_WTP == true)
    {
      feedback_msg = m_waypoint_engine.setWaypoint(m_osx, m_osy);
      m_flag_update_WTP = false;
    }
  else
    feedback_msg= m_waypoint_engine.setNextWaypoint(m_osx, m_osy);
  //--------------------------------------------------------------------//
  //--------------------------------------------------------------------//
...
void BHV_Waypoint::postStatusReport()
{
  ...
  // Modified by S. Reed and V. Schmidt, CCOM/UNH, 7/1/16
  //
  // Post the location next waypoint to the MOOSDB.
  postMessage("Next_WPT", doubleToString(m_nextpt.x())+","+doubleToString(m_nextpt.y()));
  //--------------------------------------------------------------------//
  //--------------------------------------------------------------------//
